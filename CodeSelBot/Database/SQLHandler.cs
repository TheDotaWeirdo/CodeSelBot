...
namespace SchoolSystemManager.Database
{
	...
	{
		public const string ConnectionString = @"Data Source=DESKTOP-IRDFTEA\SQL2012;Initial Catalog=CodeSelData;User ID=sa;Password=sa";
		...
		public static IEnumerable<UserWarframeSub> GetUserWarframeSubs()
			=> SqlHelper.ExecuteReader(ConnectionString, "dbo.GetWarframeSub").Select(DatabaseConvertions.UserWarframeSub);

		public static void AddUserWarframeSub(UserWarframeSub sub)
			=> SqlHelper.ExecuteScalar(ConnectionString, "dbo.AddWarframeSub", sub.UserId, sub.Type, sub.Param);

		public static void DeleteUserWarframeSub(UserWarframeSub sub)
			=> SqlHelper.ExecuteScalar(ConnectionString, "dbo.DeleteWarframeSub", sub.UserId, sub.Type, sub.Param);
